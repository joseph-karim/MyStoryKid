...
const useBookStore = create((set, get) => ({
  ...
  generateBook: async () => {
    ...
    try {
      ...
      if (!result || !result.success) { // Check result existence first
      ...
      const { book } = result; // This should now be safe if validation passed
      ...
      const newBook = {
        ...
        // Ensure we're using the data passed IN, not just from the returned book struct if metadata missing
        category: storyData.category, 
        artStyleCode: storyData.artStyleCode, 
        createdAt: new Date().toISOString(),
        updatedAt: new Date().toISOString(),
        // Add other metadata from storyData that might not be in the 'book' object from the service
      ...
      console.log('[generateBook Store] Assembled newBook object:', newBook); // Log the assembled book object
      ...
      console.log('[generateBook Store] Returning bookId:', newBook.id);
      return { bookId: newBook.id }; // Return the ID!

    } catch (error) {
      console.error('Error in generateBook store action:', error);
      console.error('[generateBook Store] Service Result was:', JSON.stringify(result)); // Log result on error too
      set({ isLoading: false });
